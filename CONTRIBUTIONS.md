...
| Storefront | <ul><li>Livewire 3 / Tailwind 4 setup and the shared layout, header, and footer</li><li>Catalogue page — search, category and brand facets with live counts, stock and sale filters, sorting, pagination</li><li>Design tokens: the `ink`/`marine` palette, two radii, two animations, `prefers-reduced-motion` honoured</li></ul> |
| Seeders | <ul><li>Role and staff account seeding</li><li>`DemoSeeder` — a full catalogue built through the Actions, not written directly</li><li>`PlaceholderImage`, deterministic SVG product art under ADR-0003's no-binaries rule</li></ul> |
