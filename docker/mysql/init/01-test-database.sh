...
# other databases the test suites point at, granting the same app user
# access to them — otherwise `pest` fails against a fresh volume with
# "Access denied ... to database 'amazoff_test'", matching what CI's
# MySQL service provisions for the same reason.
#
#   amazoff_test    — the default Feature/Unit suite (phpunit.xml).
#   amazoff_browser — the Pest browser testsuite (ADR-0017); a dedicated
#                     database because that suite does not use a refresh
#                     trait and truncates/seeds explicitly per test.
#
...
mysql -uroot -p"$MYSQL_ROOT_PASSWORD" <<-EOSQL
	...
	CREATE DATABASE IF NOT EXISTS amazoff_browser;
	GRANT ALL PRIVILEGES ON amazoff_test.* TO '$MYSQL_USER'@'%';
	GRANT ALL PRIVILEGES ON amazoff_browser.* TO '$MYSQL_USER'@'%';
