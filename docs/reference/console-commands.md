...
| `orders:expire-unpaid` | Cancels card orders left in `AwaitingPayment` past `config('orders.unpaid_ttl_minutes')` and releases the stock they hold, via `TransitionOrderStatus(Cancelled)` (ADR-0022) | The scheduler, every minute. On Railway that is meant to be a dedicated `scheduler` service running `php artisan schedule:work` (ADR-0024) — **as of 2026-09-13 that service does not exist**, so nothing calls this on its own on the beta at all, not merely "unverified." `railway ssh -- php artisan orders:expire-unpaid` is the only way it runs, by hand, until the service is created (`how-to/deploy-and-host.md`, "Worker + scheduler"). |
...
| `shipments:sync-tracking` | Queues one `App\Jobs\SyncShipmentTracking` per shipment still `Shipped`/`InTransit` with a `tracking_number` — the actual courier polling happens in the Job, run by the **worker**, not inline in this command. Same "does the service exist" caveat as `orders:expire-unpaid` above: the scheduler queues the work, but nothing sends it until the worker service also exists. | The scheduler, every 5 minutes |
