...
| Art. 17 — erasure | Anonymise the order, delete the rest | `App\Actions\Gdpr\EraseCustomer`, ADR-0019, `write-rules/gdpr.md`. **Partial gap:** copies of contact messages emailed to the shop inbox (`ContactMessageReceived`) are outside the Action's reach and must be deleted by hand |
