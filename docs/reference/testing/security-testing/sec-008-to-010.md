...
**Addendum (2026-09-13): the contact form now sends mail.** The "neither
form sends mail" bound above was true when measured and is no longer true
for `ContactForm`: each accepted submission queues `ContactMessageReceived`.
It is still not amplification in the sense that matters — the recipient is
the fixed shop inbox, never an address the submitter chooses — and the
5 / 60s limiter above caps it per IP. What changed is that flooding the form
now floods a human's inbox as well as the table. The honeypot and throttled
paths queue nothing (`ContactFormTest`).
