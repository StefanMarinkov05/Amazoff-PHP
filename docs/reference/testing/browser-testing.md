...
| `ThreeDSecureTest` | **Local-runbook-only, not run in CI.** The SCA challenge flow with a real Stripe test key: real checkout, real Stripe Elements, `4000 0025 0000 3155`, the real hosted 3DS2 challenge clicked in a real browser, then a constructed-and-signed `payment_intent.succeeded` webhook (not `stripe listen` — see `stripe-testing.md` for why) proving `payments` reaches `Paid` and `payment_events` idempotency. `->markTestSkipped()` when `STRIPE_SECRET`/`STRIPE_WEBHOOK_SECRET` are not real test-mode values. |
