...
| `<x-site.header>` | `site/header.blade.php` | — | The top nav: the logo, the `<x-site.wordmark>`, the primary links (with active-state matching), cart badge, account menu. |
...
| `<x-site.wordmark>` | `site/wordmark.blade.php` | `tone` (`dark` default, or `light`) | The "Amazoff" text mark — two-tone split plus the curled underline from `public/images/logo.png` — factored out so the header, footer, and home hero can't drift from each other. `tone="light"` is for a dark background (footer, hero); the Filament admin panel's `brand-logo.blade.php` inlines its own copy instead, since Filament compiles separate CSS and this component's `--color-brand-orange` token does not exist there. |
| `<x-site.cookie-consent>` | `site/cookie-consent.blade.php` | — | The first-visit cookie notice (ePrivacy Art. 5(3), ADR-0019). Rendered once by the app layout; hidden once `cookie_consent` holds `accepted` or `rejected` (`CookieConsent::decided()`) — any other value shows it again. Writes `accepted`/`rejected` from Alpine and hides itself with `x-show` (not `$el.remove()` — see `CookieConsentTest`); `App\Support\CookieConsent::granted()` is the read side. Nothing to gate today — only essential cookies are set. |
| `<x-site.prose-page>` | `site/prose-page.blade.php` | `title`, `standfirst` (optional), `updated` (optional), `draft` (optional bool) | The shell for the six plain informational pages (`/delivery`, `/payment-information`, `/faq`, `/terms`, `/privacy`, `/cookies`) — heading, optional standfirst and "last updated" line, body typography (headings, lists, links, tables), back-link. `draft` renders an amber "not legal advice yet" banner above the title — set on `/privacy` and `/terms` until counsel has reviewed them (ADR-0019). `/about` deliberately does **not** use it (it has its own hero). |
