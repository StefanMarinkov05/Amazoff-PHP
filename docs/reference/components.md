...
| `<x-site.cookie-consent>` | `site/cookie-consent.blade.php` | — | The first-visit cookie notice (ePrivacy Art. 5(3), ADR-0019). Rendered once by the app layout; hidden the moment a `cookie_consent` cookie is present. Writes `accepted`/`rejected` from Alpine; `App\Support\CookieConsent::granted()` is the read side. Nothing to gate today — only essential cookies are set. |
